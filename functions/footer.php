		...
		<div class="row add-bottom-25">
				...
				<ul class="footer-selector clearfix">
					<li>
						<div class="styled-select lang-selector">
							<select id="selectLanguage" onchange="var currentDate = new Date(); var currentTime = currentDate.getTime(); var expireTime = currentTime + 31556952000; currentDate.setTime(expireTime); document.cookie = 'TOPSHOPLANGUAGE=' + this.value + '; expires=' + currentDate.toUTCString(); location.reload()">
								<option value="English" <?php if ($_COOKIE["TOPSHOPLANGUAGE"] == "English") { echo "selected"; } ?>>English</option>
								<option value="French" <?php if ($_COOKIE["TOPSHOPLANGUAGE"] == "French") { echo "selected"; } ?>>Français</option>
								<option value="Creole" <?php if ($_COOKIE["TOPSHOPLANGUAGE"] == "Creole") { echo "selected"; } ?>>Kreol</option>
							</select>
						</div>
					</li>
					<li>
						<div class="styled-select currency-selector">
							<select id="selectCurrency" onchange="var currentDate = new Date(); var currentTime = currentDate.getTime(); var expireTime = currentTime + 31556952000; currentDate.setTime(expireTime); document.cookie = 'TOPSHOPCURRENCY=' + this.value + '; expires=' + currentDate.toUTCString(); location.reload()">
								<option value="MUR" <?php if ($_COOKIE["TOPSHOPCURRENCY"] == "MUR") { echo "selected"; } ?>>Mauritian Rupee (MUR)</option>
								<option value="USD" <?php if ($_COOKIE["TOPSHOPCURRENCY"] == "USD") { echo "selected"; } ?>>United States Dollar (USD)</option>
								<option value="EUR" <?php if ($_COOKIE["TOPSHOPCURRENCY"] == "EUR") { echo "selected"; } ?>>Euro (EUR)</option>
							</select>
						</div>
					</li>
			...
			<div class="col-lg-6">
				<ul class="additional-links">
					...
					<li><a href="https://bit.do/abhp" target="_blank" rel="noreferrer" onclick="alert('Hello World!')">Abhijeet Pitumbur &copy; Topshop 2022</a></li>
				</ul>
			</div>
		</div>
	</div>
